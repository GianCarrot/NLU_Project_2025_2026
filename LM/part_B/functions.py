...
def train_loop(data, optimizer, model, device, accumulation_steps=4):
    model.train()
    loss_array = []
    number_of_tokens = []

    # Scaler per la precisione mista
    ...
    for i, (input_ids, labels, n_tokens) in enumerate(pbar):
        ...
        # Autocast forza i calcoli in fp16
...
def run_pipeline(model, train_loader, dev_loader, test_loader, device, lr=3e-4, n_epochs=100, patience=3, accumulation_steps=4):
    """
    ...
    for epoch in range(n_epochs):
        ...
        train_loss = train_loop(train_loader, optimizer, model, device, accumulation_steps=accumulation_steps)
