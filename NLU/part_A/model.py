...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # Calcolo 유사ità scalata usando math.sqrt per evitare problemi di Device
        similarity = (q @ k.transpose(-2, -1)) / math.sqrt(self.h_dim)

        # Applicazione maschera causale
        similarity = similarity.masked_fill(mask[:, :, :L, :L] == 0, float('-inf'))
...
class GPT2(nn.Module):
    def __init__(
        ...
        self.drop_out = nn.Dropout(out_dropout)
    ...
    def forward(self, idx, seq_lens):
        ...
        x = self.ln_f(x)
        x_dropped = self.drop_out(x)
        slots = self.slot_out(x_dropped)

        batch_indices = torch.arange(x.shape[0], device=x.device)
        cls_tokens = x_dropped[batch_indices, seq_lens - 1]
